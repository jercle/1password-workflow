...
function copyByLabel(label, itemID) {
  const value = runOP('item', 'get', '--field', `label=${label}`, itemID)[
    'value'
  ];
  copySensitive(value);
}
...
// String -> ()
function showByLabel(label, itemID) {
  const value = runOP('item', 'get', '--field', `label=${label}`, itemID)[
    'value'
  ];
  copySensitive(value);
...
function getItems(userID, excludedVaults) {
  ...
  const account = runOP('account', 'list').find(
    (account) => account['user_uuid'] === userID
  );
  const accountURL =
    envVar('hostnames_only') === '1'
      ? getHostname(account['url'])
      : account['url'];

  return runOP('item', 'list', '--account', userID)
    .map((item) => {
      ...
      const displayURL =
        envVar('hostnames_only') === '1' ? getHostname(url) : url;

      return {
        uid: item['id'],
        title: item['title'],
        subtitle: url
          ? `${displayURL} 𐄁 ${vaultName} 𐄁 ${accountURL}`
...
function getExcluded(filePath, varName) {
  ...
  return readJSON(filePath)
    ['items'].filter((item) => item['variables']['excluded'])
...
function cliValidInstall() {
  ...
  if (parseInt(runCommand([cliPath, '--version']).split('.')[0]) < 2)
    return false;
...
function cliAppUnlockEnabled() {
  ...
  if (!$.NSFileManager.defaultManager.fileExistsAtPath(settingsFile))
    return false;
  if (readJSON(settingsFile)['developers.cliSharedLockState.enabled'])
    return true;
  return false;
}
...
function readJSON(filePath) {
  ...
  const string = $.NSString.alloc.initWithDataEncoding(
    data,
    $.NSUTF8StringEncoding
  ).js;
...
function run(argv) {
  ...
  switch (argv[0]) {
    case 'sanity_checks':
      return; // Stop if we only want to check everything is ready
    case 'op_path':
      return opPath();
  }
  ...
  const sfUserIDs = usersObject.concat({
    title: 'Update items',
    arg: 'update_items',
  ...
  switch (argv[0]) {
    case 'update_items':
      ...
      const sfItems = activeUserIDs.flatMap((userID) =>
        getItems(userID, excludedVaults)
      );

      const sfVaults = activeUserIDs
        .flatMap((userID) => getVaults(userID, excludedVaults))
        .concat({
          title: 'Update items',
          arg: 'update_items',
          variables: { excluded: false, vaultID: false }, // Avoid "undefined" errors in fuctions which interact with vaults file
    ...
    case 'show_otp':
      return showOTP(argv[1]);
    case 'copy_by_label':
      return copyByLabel(argv[1], argv[2]);
    case 'show_by_label':
      return showByLabel(argv[1], argv[2]);
